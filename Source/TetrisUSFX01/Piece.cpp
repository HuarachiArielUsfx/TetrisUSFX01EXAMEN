    ...
    TArray<ABlock*> Blocks;
     ...
     SpawnBlocks() {
        ...
        for (auto&& YZ : YZs)
        {
            ...
            B->AttachToActor(this, FAttachmentTransformRules::KeepRelativeTransform);
            B->SetActorRelativeLocation(FVector(0.0, YZ.first, YZ.second));
            if (YZ.second >= MaxHeight) { // MaxHeight es la posición Y del tope del juego
                bGameEnded = true;
            }
        }
    }
...
void APiece::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    if (bReachedLimit)
    {
        UE_LOG(LogTemp, Warning, TEXT("Pieza ha llegado al límite."));

        // Reiniciar el juego aquí
        // Puedes reiniciar el juego llamando a una función o reiniciando las variables necesarias
        // para reiniciar el juego, como reiniciar la posición de las piezas y limpiar los bloques en el tablero.
    }
    else
    {
        UE_LOG(LogTemp, Warning, TEXT("Pieza eliminada."));
    }
}*/
